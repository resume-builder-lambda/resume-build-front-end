...
export class AssignmentUpload extends SampleBase {
	constructor(props) {
		super(props);
		...
		document.getElementById('customdropArea').onclick = (args) => {
			...
			if (target.classList.contains('e-file-delete-btn')) {
				for (let i = 0; i < this.uploadObj.getFilesData().length; i++) {
					if (
						target.parentElement.parentElement.getAttribute('data-file-name') ===
						...
						<div className="customdrop_wrapper">
							<div className="dropArea_wrap" id="customTarget">
								<div className="font-icons">
									<span className="e-icons sf-icon-pdf" />
									<span className="e-icons sf-icon-txt" />
									<span className="e-icons sf-icon-png" />
								</div>
							...
							<div id="customdropArea">
								<span id="drop" className="customdropArea">
									<a href="https://google.com" id="browse">
										<u>Browse</u>
									</a>{' '}
								</span>
